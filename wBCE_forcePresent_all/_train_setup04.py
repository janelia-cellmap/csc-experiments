# %%
from cellmap_models.pytorch import cosem
import torch
...
model_to_load = model_name = (
    f"setup04.1820500_{random_seed}_8nmOut.all"  # name of the model for saving
)
load_model = "best"  # load the latest model or the best validation model
base_model = cosem.load_model("setup04/1820500")  # load the model
backbone = base_model.backbone[0]
...
model = torch.nn.Sequential(
    backbone,
    torch.nn.Conv3d(
        base_model.final_feature_width, base_model.final_feature_width, kernel_size=3
    ),
    torch.nn.ReLU(),
    torch.nn.Conv3d(
        base_model.final_feature_width, base_model.final_feature_width, kernel_size=3
    ...
    torch.nn.ReLU(),
    torch.nn.Conv3d(base_model.final_feature_width, len(classes), kernel_size=1),
)

input_array_info = {
    "shape": base_model.min_input_shape,
    "scale": base_model.voxel_size_input,
...
target_array_info = {
    "shape": model(
        torch.ones(
            ...
            + base_model.min_input_shape
        )
    ).shape[
        -3:
    ],  # shape of the data to load for the target
    "scale": base_model.voxel_size_input,  # took out the upsample layer
    # "scale": base_model.voxel_size_output,
